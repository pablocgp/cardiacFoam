...
\IfFileExists{#2}{\includegraphics[#1]{#2}}{\fbox{\parbox[c][0.28\textheight][c]{0.92\linewidth}{\centering Missing figure: \texttt{\detokenize{#2}}\\Run the corresponding \code{plot\_convergence\_*.m} script in this report directory.}}}%
}
...
the mass matrix can be \code{lumped} or high-order \code{consistent}. Linear
...
JFNK uses a persistent matrix-free \code{MatShell}+KSP pair with restarted
GMRES and an optional preconditioner derived from $\Aimp$ with a local
...
data driven: tables are generated from the current tutorial outputs and the
plotted figures can be regenerated with the supplied MATLAB scripts.
\end{abstract}
...
$\Mmass$ & mass matrix & \code{M} (moved into \code{BImplicit} after $\Aimp/\Bimp$) \\
...
legacy name \code{LRECoeffs\_states} is still accepted for the latter, but the
solver prints a warning and prefers \code{LRECoeffs\_Iion}.
...
underlying GMRES Krylov subspace is reused. Numerically this is identical to
...
\code{PetscKspMatrixSolver} above. Selecting \code{jfnkPreconditioner=none}
disables the left preconditioner and runs unpreconditioned GMRES on the
...
exist in $\Aimp$ (both $\Mmass$ and $\Kstif$ have non-zero diagonals).
...
\code{highOrderCoeffs/LRECoeffs\_Iion} & LRE settings \code{N}, \code{Nn}, \code{k}, and \code{maxStencilSize} for ionic quadrature. The legacy \code{LRECoeffs\_states} name is accepted with a warning. \\
...
references. The 2D data are generated from
...
All result blocks in this section use \code{crankNicolson} time integration and
the \code{consistent} mass matrix, and are split by nonlinear method, mesh
family, stabilisation parameter, and time step. The 2D orders are fitted over
$N=10$--$100$ and $N=70$--$100$; the 3D orders are fitted over $N=10$--$30$
and $N=20$--$30$. The figures can be regenerated with
\code{plot\_convergence\_2D.m} and \code{plot\_convergence\_3D.m}; missing
figure boxes indicate plots that have not yet been generated from the latest
data.
