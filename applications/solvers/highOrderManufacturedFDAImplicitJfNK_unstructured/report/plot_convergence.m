...
% for the implicit manufactured FDA JfNK/Picard/diagonalIion runs.
%
% One figure file is generated for each dimension, nonlinear method, time
% step, time scheme, mass matrix, variable, and error metric. The legend is
% placed inside the lower-left corner of the axes and includes the observed
% convergence order over N=10--100, the observed order over N=70--100, and
% the accumulated runtime and peak-RSS cost over the mesh sweep represented
% by each curve.
%
...
close all; clc;
baseRoot = '/home/pablo/OpenFOAM/pablo-v2312/run/tutorials_electro/highOrderManufacturedFDAImplicitJfNK/results/example0';
    ...
    for imethod = 1:numel(methods)
        ...
        for ischeme = 1:numel(schemes)
            ...
            for imass = 1:numel(masses)
                mass = masses{imass};
                commonAxes = precomputeCommonAxes(resultRoot, dtTags, method, scheme, mass, metricNames);

                for idt = 1:numel(dtTags)
                    dtTag = dtTags{idt};
                    files = findErrorFiles(resultRoot, dtTag, method, scheme, mass);
                    if isempty(files)
                        continue;
                    end
...
function files = findErrorFiles(resultRoot, dtTag, method, scheme, mass)
...
function commonAxes = precomputeCommonAxes(resultRoot, dtTags, method, scheme, mass, metricNames)
...
for ifield = 1:size(metricNames, 1)
    for imetric = 1:2
        ...
        allE = [];
        for idt = 1:numel(dtTags)
            files = findErrorFiles(resultRoot, dtTags{idt}, method, scheme, mass);
            for i = 1:numel(files)
...
for i = 1:numel(files)
    ...
    [orderAll, orderFine] = convergenceOrders(T, metric);
...
function [orderAll, orderFine] = convergenceOrders(T, metric)
orderAll = NaN;
orderFine = NaN;
if ~ismember(metric, T.Properties.VariableNames)
    return;
...
N = meshN(T);
e = double(T.(metric));
orderAll = fitConvergenceOrder(N, e, 10, 100);
orderFine = fitConvergenceOrder(N, e, 70, 100);
end
