...
namespace
{
    ...
    {
        return 0.5*sqr(u2)*(u1 + 2.0*u3 - 2.0*V);
    }
    ...
    )
    {
        ...
        const label mMax = min(max(maxIterations, label(1)), label(n));

        EigVec x = EigVec::Zero(n);
        const scalar beta = b.norm();
        iterations = 0;
        estimatedError = beta;
        ...
        std::vector<EigVec> V(mMax + 1, EigVec::Zero(n));
        ...
        for (label j = 0; j < mMax; ++j)
        {
            ...
            EigVec xj = EigVec::Zero(n);
            for (label i = 0; i <= j; ++i)
            {
                xj += y[i]*V[i];
            }

            const scalar relResidual = (matVec(xj) - b).norm()/max(beta, SMALL);
            bestX = xj;
            iterations = j + 1;
    ...
    void computeVmSourceDerivativeFromIntegrationPoints
    (
        const scalarField& VmIntegrationPoints,
        const scalarField& u1IntegrationPoints,
        const scalarField& u2IntegrationPoints,
        const scalarField& u3IntegrationPoints,
        scalarField& sourceDerivativeIntegrationPoints
    )
    {
        forAll(sourceDerivativeIntegrationPoints, integrationPointI)
        {
            sourceDerivativeIntegrationPoints[integrationPointI] =
                dVmSplitSourceDV
                (
                    VmIntegrationPoints[integrationPointI],
                    u1IntegrationPoints[integrationPointI],
    ...
    void computeCellCentredVmSourceDerivative
    (
        const volScalarField& Vm,
        const volScalarField& u1,
        const volScalarField& u2,
    ...
    {
        const scalar s = (dt > SMALL ? min(max(tau/dt, scalar(0)), scalar(1)) : 1.0);
        const scalar V = (1.0 - s)*VmOld + s*VmNew;
        reactionRates(V, u1, u2, u3, du1dt, du2dt, du3dt);
    }
    ...
    )
    {
        ...
        err = max
        (
            mag(u1Fifth - u1Fourth)/(mag(u1Fifth) + SMALL),
            max
            (
                mag(u2Fifth - u2Fourth)/(mag(u2Fifth) + SMALL),
                mag(u3Fifth - u3Fourth)/(mag(u3Fifth) + SMALL)
            )
        );
    }
    ...
    (
        ...
        const scalar stateODETolerance,
        const label stateODEMaxSubSteps,
        scalar& u1New,
        scalar& u2New,
        scalar& u3New
    )
    {
        ...
        scalar u2Cur = u2Old;
        scalar u3Cur = u3Old;
        const scalar tolerance = max(stateODETolerance, scalar(SMALL));
        const scalar hMin = max(dt*1.0e-12, SMALL);

        for (label subStep = 0; subStep < max(stateODEMaxSubSteps, label(1)); ++subStep)
        {
            if (tau >= dt - SMALL)
            {
                break;
            ...
            (
                scalar(4.0),
                max(scalar(0.1), scalar(0.84)*std::pow(tolerance/(err + SMALL), 0.25))
            );
        ...
        {
            WarningInFunction
                << "State RKF45 reached maxSubSteps before completing dt. "
                << "tau = " << tau << ", dt = " << dt << nl;
        }
...
int main(int argc, char* argv[])
{
    ...
    Info<< "Running high-order manufactured FDA implicit JfNK solver" << nl
        << "Dimension = " << dim << nl
        << "beta = " << beta << nl
        << "dt = " << dt << nl
    ...
    if (writeNonlinearResiduals)
    {
        ...
        (
            runTime.path()/"postProcessing"/"highOrderManufacturedFDAImplicitJfNK"
        );
        mkDir(residualDir);
        ...
        nonlinearResidualFilePtr()
            ...
            << "newtonResidual Vm_relL2 u1_relL2 u2_relL2 Iion_relL2 converged"
            << nl;
    }
    ...
    while (runTime.value() < runTime.endTime().value() - SMALL)
    {
        ...
        auto computeSourceDerivative = [&]()
        {
            ...
            {
                computeCellCentredVmSourceDerivative
                (
                    VmGuess,
                    u1Guess,
                    u2Guess,
        ...
        if (useJFNK)
        {
            ...
            auto residualFor = [&](const EigVec& xCandidate, const bool updateGuess)
            {
                if (updateGuess)
                {
                    ...
                    EigVec residual = AImplicit*xApplied - rhsFromSource(sourceVm);
                    return residual;
                }
                ...
                EigVec residual = AImplicit*xApplied - rhsFromSource(sourceTmp);
                return residual;
            };
            ...
            )
            {
                ...
                auto matVec = [&](const EigVec& v)
                {
                    const scalar eps = epsScale/max(v.norm(), SMALL);
                    EigVec Jv = (residualFor(x + eps*v, false) - R)/eps;
                    return Jv;
                };
                ...
                const EigVec Rnew = residualFor(x, true);
                const EigVec rhsNew = rhsFromSource(sourceVm);
                const scalar newtonResidualNew = relativeL2Norm(Rnew, rhsNew);
                ...
                const bool converged =
                 ...
                 && u1Residual <= nonlinearU1Tolerance
                 && u2Residual <= nonlinearU2Tolerance
