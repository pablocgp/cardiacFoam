...
% implicit high-order heat-equation MMS example5 runs.
close all; clc;
baseRoot = '/home/pablo/OpenFOAM/pablo-v2312/run/tutorials_electro/testHighOrderHeatEquationImplicit/results/example5';
figDir = fullfile(fileparts(mfilename('fullpath')), 'figures');
if ~exist(figDir, 'dir'); mkdir(figDir); end
files = dir(fullfile(baseRoot, '*', 'hoT_*_scheme_*_mass_*', 'endT_*_dt_*', 'errors_vs_N_dt_*.dat'));
keys = {};
for i = 1:numel(files)
    fullName = fullfile(files(i).folder, files(i).name);
    parts = split(string(fullName), filesep);
    dimTag = char(parts(find(parts == "example5") + 1));
    cfg = char(parts(find(parts == "example5") + 2));
    dtTok = regexp(files(i).name, 'errors_vs_N_dt_(.+)\.dat', 'tokens', 'once');
    if ~isempty(dtTok); keys{end+1} = [dimTag '|' cfg '|' dtTok{1}]; end %#ok<AGROW>
end
keys = unique(keys);
for k = 1:numel(keys)
    parts = split(keys{k}, '|'); dimTag = parts{1}; cfg = parts{2}; dtTag = parts{3};
    runFiles = dir(fullfile(baseRoot, dimTag, cfg, ['endT_*_dt_' dtTag], ['errors_vs_N_dt_' dtTag '.dat']));
    token = regexp(cfg, 'hoT_.+_scheme_(.+)_mass_(.+)', 'tokens', 'once');
    scheme = token{1}; mass = token{2};
    plotMetric(runFiles, 'L2', sprintf('heat_implicit_%s_%s_%s_T_L2_dt_%s', dimTag, scheme, mass, dtTag), '$||T-T^*||_{L_2}$');
    plotMetric(runFiles, 'G2', sprintf('heat_implicit_%s_%s_%s_T_G2_dt_%s', dimTag, scheme, mass, dtTag), '$||T_h-T^*||_{G_2}$');
end
end
...
for i=1:numel(files)
    ...
    level = regexp(files(i).folder, 'hoT_([^_]+)_scheme', 'tokens', 'once'); if isempty(level); level={'unknown'}; end; level=level{1};
