...
cell $\Omegac$ with a high-order quadrature at points $\{x_q\}$ and normalised
weights $\{w_q\}$ ($\sum_q w_q=1$; the LRE weights already carry the sub-cell
volume fractions, so the quadrature returns the cell \emph{average} directly),
\begin{equation}
  \overline{\Iion}_{,\,c}
...
Let $\avg{\cdot}=\sum_q w_q(\cdot)$ (with $\sum_q w_q=1$) denote the
cell-quadrature average, so $\Vbarc=\avg{\Vm}$. \gpode\ evaluates the kinetics pointwise and
