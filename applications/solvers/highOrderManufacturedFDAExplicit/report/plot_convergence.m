...
% for the explicit manufactured FDA runs.
%
% One figure file is generated for each dimension, time step, variable, and
% error metric. The legend is placed outside the axes so it does not hide the
% convergence curves.
%
% NOTE: the result directory names still use the legacy token "hoStates".
% In the current solver this token denotes the high-order quadrature level
% used for Iion and for the local ODE state values stored at the Iion
% integration points.

close all; clc;
baseRoot = '/home/pablo/OpenFOAM/pablo-v2312/run/tutorials_electro/highOrderManufacturedFDAExplicit/results/example0';
figDir = fullfile(fileparts(mfilename('fullpath')), 'figures');
if ~exist(figDir, 'dir'); mkdir(figDir); end

dimTags = {'2D', '3D'};
dtTags = {'1p0em03', '1p0em04', '1p0em05', '1p0em06'};
fields = {'Vm', 'u1', 'u2'};
...
for idim = 1:numel(dimTags)
    ...
    commonAxes = precomputeCommonAxes(resultRoot, dtTags, metricNames);

    for idt = 1:numel(dtTags)
        dtTag = dtTags{idt};
        files = findErrorFiles(resultRoot, dtTag);
        if isempty(files)
            warning('No errors_vs_N file found for %s, dt tag %s', dimTag, dtTag);
            continue;
        end
        ...
        for ifield = 1:numel(fields)
            for imetric = 1:2
                ...
                title(ax, sprintf('Explicit manufactured FDA: %s, %s, dt = %s', dimTag, panelTitle, prettyDt(dtTag)), ...
                    'Interpreter', 'tex');

                outBase = fullfile(figDir, sprintf('explicit_%s_%s_%s_dt_%s', dimTag, fields{ifield}, metricSuffix{imetric}, dtTag));
...
function commonAxes = precomputeCommonAxes(resultRoot, dtTags, metricNames)
% Common limits are computed per dimension, variable, and metric over all
% requested time steps. This keeps the four dt plots visually comparable.
...
for ifield = 1:size(metricNames, 1)
    for imetric = 1:2
        ...
        allE = [];
        for idt = 1:numel(dtTags)
            files = findErrorFiles(resultRoot, dtTags{idt});
            for i = 1:numel(files)
...
% Use decade limits in y, but only the real h-range in x. This avoids 3D
% plots with N=10..20 being displayed as if they included the 2D N=100 data.
...
% Draw fixed reference slopes for a given dimension/variable/metric family.
% With XDir reversed, positive powers h^p appear decreasing from left to right.
if isempty(lims)
    return;
end
xMin = lims(1);
...
% Prefer the N encoded in the data-file name, falling back to the N column.
% This protects 2D/3D plots from accidental stale table metadata.
N = T.N;
if ismember('dat_file', T.Properties.VariableNames)
    fileNames = string(T.dat_file);
    parsedN = nan(size(N));
    for i = 1:numel(fileNames)
        token = regexp(fileNames(i), '_(\d+)_cells', 'tokens', 'once');
        if ~isempty(token)
...
inlineE = {};
inlineLabels = {};
inlineColors = zeros(numel(files), 3);
inlineCount = 0;
...
label = strrep(label, 'hoStates', 'hoIion');
end
...
tokens = regexp(configName, '^hoVm_(.+)_hoStates_(.+)$', 'tokens', 'once');
if isempty(tokens)
