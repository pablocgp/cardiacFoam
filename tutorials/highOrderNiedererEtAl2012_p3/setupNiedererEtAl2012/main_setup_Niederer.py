...
"""
"""
Main pipeline controller for NiedererEtAl2011 benchmark.
...
================================================================================
Main Setup Pipeline for Niederer Et Al. 2011 Slab Benchmark
================================================================================
...
def stage_simulations():
    ...
    sweep_script = Path(CONFIG["case_root"]) / "setupNiedererEtAl2011" / "simulation" / "setup_multiple_simulations_Niederer.py"
...
def stage_postprocess():
    ...
    line_script = load_module(Path(CONFIG["case_root"])/ "setupNiedererEtAl2011" / "postProcessing" / "line_postProcessing.py")
    line_script.plot_line_csvs(
        folder=Path(CONFIG["case_root"]) / OUTPUT_FOLDER,
        excel_path=Path(CONFIG["case_root"]) / "setupNiedererEtAl2011" /CONFIG["excel_reference"]
    )

    print("\n--- Running Points Postprocessing ---\n")
    points_script = load_module(Path(CONFIG["case_root"])/ "setupNiedererEtAl2011" / "postProcessing" / "points_postProcessing.py")
...
def build_cli():
    parser = argparse.ArgumentParser(description="Niederer Et Al. 2011 Automation")
